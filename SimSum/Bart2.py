...
class SumSim(pl.LightningModule):
    def __init__(self,args):
        ...
        # self.W = nn.Parameter(torch.randn(768,768))
        # self.relu = nn.ReLU()
    def is_logger(self):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # if 'logits' in sum_outputs:
        #     # Decode logits to get the actual summaries
        #     logits = sum_outputs.logits
        #     predicted_ids = torch.argmax(logits, dim=-1)
        #     predicted_summaries = [self.summarizer_tokenizer.decode(ids, skip_special_tokens=True) for ids in predicted_ids]
        ...
        # print(summary_ids[0])
        # decoded = self.summarizer_tokenizer.decode(summary_ids[0],skip_special_tokens=True)
        # print(decoded)
        # print(decoded[0])
        ### Original loss
    ...
    def add_model_specific_args(parent_parser):
      ...
      p.add_argument('-TrainBS','--train_batch_size',type=int, default=16)
      p.add_argument('-ValidBS','--valid_batch_size',type=int, default=16)
      p.add_argument('-lr','--learning_rate',type=float, default=5e-4)
      p.add_argument('-MaxSeqLen','--max_seq_length',type=int, default=256)
      p.add_argument('-AdamEps','--adam_epsilon', default=1e-8)
      p.add_argument('-WeightDecay','--weight_decay', default = 5e-5)
...
class TrainDataset(Dataset):
    ...
    def _load_data(self):
        ...
        # self.inputs = self.inputs[:90]
        # self.targets = self.targets[:90]
    def __len__(self):
...
def train(args):
    ...
    # model = SumSim.load_from_checkpoint("/home/tejadhith/Project/NLP/easy-summary/SimSum/experiments/exp_SIMSUM_WIKI_AUTO_REDUCED1713981629570149/checkpoint-epoch=5.ckpt")
    # model = SumSim.load_from_checkpoint("/home/tejadhith/Project/NLP/easy-summary/SimSum/experiments/exp_SIMSUM_WIKI_AUTO_REDUCED1714133569750998/checkpoint-epoch=7.ckpt")
