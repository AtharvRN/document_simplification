...
def run_generator(model_ckpt, test_set, x_col="complex", op_col=None, reading_lvl=None,
                    ...
                    num_workers=16, beams=5, length_penalty=1.0, min_length=False, silent=False,
    ...
    with torch.no_grad():
        if op_col is not None or reading_lvl is not None:
            # add control tokens
            print(test_set.iloc[0][x_col])
            # print(test_set.iloc[0]["complex"])
            # print(test_set.iloc[0]["simple"])
